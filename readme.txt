...
- Maintenance: fixed infrastructure tests that overwrote the WordPress table prefix.
...
- Change: **Add PDF Header** is now enabled by default on new installations. PDF header generation runs entirely in the recipient's browser, so there is no server memory cost. Existing sites keep their saved preference.

### 26.05.19.1

- Change: when **Add PDF Header** is enabled, the From/To header is now generated in the recipient's browser using pdf-lib instead of server-side TCPDF/FPDI. This removes the server memory cost entirely.
- Fix: when printing a multi-page PDF letter, all pages are now sent to the printer (previously only the first page printed, regardless of whether the header was enabled).
- Fix: the print tab now closes reliably after the print dialog is dismissed.
...
- Fix: prevent third-party widgets (notably Google reCAPTCHA badges) from appearing in the letter print preview on sites with security/anti-spam plugins active. The print page no longer fires the global `wp_enqueue_scripts` action and only prints its own stylesheet and script.
...
- Fix: a missing bump in the database version caused the databases not to update with new needed fields. Now fixed.
